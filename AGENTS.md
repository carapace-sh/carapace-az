...
- **Man page enrichment from learn.microsoft.com**: The automated pipeline generates baseline docs from knack help text. On top of that, `documentation.flag` entries (with accepted values, defaults, and parameter groups), `documentation.command` entries, and `examples:` in ~5,658 files were manually enriched by scraping the official Azure CLI reference pages at `https://learn.microsoft.com/en-us/cli/azure/<group>` and their sub-pages (e.g. `/cli/azure/network/vnet`). This is a one-time manual step — `carapace-man update` preserves existing documentation on re-generation, so the enriched docs survive automated pipeline runs. To re-enrich after major az CLI updates, fetch the relevant group pages and sub-pages, extract command descriptions, flag docs (including metadata), and examples, and write them into the `man/cmd/az/*.yaml` files.
- **Sub-page coverage**: The enrichment follows sub-page links from each group page. For example, `network` has ~100 sub-pages (network/vnet, network/lb, network/nsg, etc.) containing the actual command documentation. This is why top-level-only scraping misses most commands — the detailed docs (parameters, examples) live on sub-pages.
